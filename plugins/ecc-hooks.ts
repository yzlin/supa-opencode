...
function resolveFileRefs(template: string): string {
  return template.replace(/\{file:([^}]+)\}/g, (_, relPath) => readPluginFile(relPath))
}
...
const ECCHooksPlugin = async ({
  ...
  const hookEnabled = (hookId: string, requiredProfile: HookProfile | HookProfile[] = "standard"): boolean => {
    ...
    const required = Array.isArray(requiredProfile) ? requiredProfile : [requiredProfile]
    return required.some((p) => profileOrder[currentProfile] >= profileOrder[p])
  }
  ...
  return {
    ...
    "tool.execute.after": async (
      ...
      if (
        ...
        try {
          ...
          if (err.stdout) {
            err.stdout.split("\n").slice(0, 5).forEach((line) => log("warn", `  ${line}`))
          }
        }
      }
      ...
      if (hookEnabled("post:bash:pr-created", ["standard", "strict"]) && input.tool === "bash") {
    ...
    "tool.execute.before": async (
      ...
      if (hookEnabled("pre:bash:git-push-reminder", "strict") && input.tool === "bash" && cmd.includes("git push")) {
      ...
      if (hookEnabled("pre:bash:long-running-reminder", "strict") && input.tool === "bash") {
        if (
    ...
    "permission.ask": async (
      input: { id: string; type: string; sessionID: string; messageID: string; callID?: string; title: string; metadata: Record<string, unknown>; pattern?: string | string[] },
      ...
      if (tool === "bash" && /^(npm test|npx vitest|npx jest|pytest|go test|cargo test)/.test(cmd)) {
        output.status = "allow"
        return
      }
    },
    ...
    "config": async (config) => {
      const ocJson = JSON.parse(readPluginFile("opencode.json")) as {
        ...
        agent?: Record<string, { prompt?: string; description?: string; model?: string; mode?: "subagent" | "primary" | "all"; tools?: Record<string, boolean> }>
        instructions?: string[]
      }
      ...
      for (const [name, def] of Object.entries(ocJson.command ?? {})) {
        ...
        config.command[name] = { ...def, template: resolveFileRefs(def.template) }
      }
      ...
      // Inject instructions (skills) as absolute paths
